...
class Enemy:
    ...
    def create_boss(self, stage, room_number):
        if room_number == 4:
            ...
            damage_scaling = 30 + (stage * 3) + (room_number * random.randint(3,5))
            defense_scaling = 10 + (stage * 2) + (room_number * random.randint(4,8))
            dodge_chance = random.uniform(0.2, 0.3)
            can_heal = random.random() < 0.2
        else:
            health_scaling = 450 + (stage * random.randint(30,70)) + (room_number * 320)
            damage_scaling = 40 + (stage * 30) + (room_number * random.randint(15,20))
            defense_scaling = 50 + (stage * 10) + (room_number * random.randint(16,30))
